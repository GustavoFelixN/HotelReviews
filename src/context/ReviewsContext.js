...
export const ReviewsContextProvider = ({ children }) => {
	...
	const fetchReviews = useCallback(async (hotelId) => {
		try {
			const data = await fetch(
				`https://my-json-server.typicode.com/GustavoFelixN/HotelReviews/hotels/${hotelId}/reviews`,
			);
	...
	const addReview = useCallback(
		async ({ hotelId, title, description, rating }) => {
			...
			try {
				const data = await fetch(
					`https://my-json-server.typicode.com/GustavoFelixN/HotelReviews/reviews`,
					{
						method: 'POST',
						body: JSON.stringify({
							id: reviewId,
